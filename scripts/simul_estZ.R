...
################### Sparse Bayesian Infinite Factor Model ####################
  ...
  p0 <- 0.4
  a_p0 = b_p0 <- 10 # hyperparameter for the prior of p0
  a_sigma <- 1
  b_sigma <- 0.3
  ...
  tau <- cumprod(delta)
  cube_Z <- array(0, dim=c(N, k_ast, num_slice))
  ...
  for (iter in 1:num_iter){
    ...
    if (iter > num_burn && iter %% thin == 0){
      ...
      k_curr <- ncol(eta)
      if (k_prev < k_curr) {
        cube_Z <- abind::abind(cube_Z,
                                 array(0, dim=c(N, k_curr-k_prev, num_slice)),
                                 along=2)
        cube_Z[ , , (iter-num_burn)/thin] <- Z
      } else {
        cube_Z[ , 1:k_curr, (iter-num_burn)/thin] <- Z
      }
    }
  ...
  Z_hat <- apply(cube_Z, 1, function(arr){
    apply(arr, 1, median)
  }) %>% t
  Z0 <- ifelse(eta0!=0, 1, 0)
