...
for (iter in 1:num_iter){
  ...
  for(i in 1:N){
    for(k in 1:k_ast){
      if (Z[i, k]){
        ...
        eta[i, k] <- MH(func=pos_eta, step_size=0.3, init=eta[i, k], M=2, a=a, b=b)
      } else {
        eta[i, k] <- 0
      }
    }
  ...
  RV_eta_s[iter] <- RVadj(eta, data[[1]]$eta)
...
plot(RV_L, type = "l")
plot(RV_L_2, type = "l")
abline(h=RV_Lambda, col=2)
legend("bottomright", legend=round(RV_Lambda,4))

plot(RV_eta_s, type = "l")
plot(RV_eta_2, type = "l")
abline(h=RV_eta, col=2)
legend("bottomright", legend=round(RV_eta,4))

plot(RV_L_eta, type = "l")
abline(h=RV_mult, col=2)
legend("bottomright", legend=round(RV_mult,4))
